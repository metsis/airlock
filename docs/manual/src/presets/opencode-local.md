...
The `opencode-local` preset bundles the sandbox setup for the
...
This is the **privacy-first** option: nothing your agent does leaves your
machine. Unlike the cloud agent presets — including the generic
[`opencode`](./opencode.md) preset — this one opens no outbound network
access. airlock stays deny-by-default; the only thing it lets through is a
loopback bridge to the inference server already running on your host.

> Use this preset when you want a fully local, no-egress agent. If you run
> opencode against a cloud provider (Anthropic, OpenAI, OpenRouter, …), use
> the [`opencode`](./opencode.md) preset instead.
...
- **Persists your session data.** `~/.local/share/opencode` (sessions,
  projects, `auth.json`) is mapped to `~/.airlock/opencode-local/data/` on the
  host, and the seeded config to `~/.airlock/opencode-local/opencode.json`, so
  both carry over between runs.
...
host, edit `~/.airlock/opencode-local/opencode.json` on the host — your
changes persist and the sandbox picks them up on the next run.
