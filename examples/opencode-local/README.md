...
The `opencode-local` preset forwards the common host ports into the sandbox
...
(`resolution = "auto"`), applies the `opencode-local` preset (host port
forwards + config/data mounts), and starts opencode. Pick a model from one of
the seeded providers — e.g. the Ollama one.
...
`~/.airlock/opencode-local/opencode.json` — your edits survive across runs.
Make the model IDs match what you've pulled/loaded on the host.
